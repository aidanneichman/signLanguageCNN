...
Original file is located at
    https://colab.research.google.com/drive/12NNHKRy6n8_f7gtdHB7yKSgaFcEos_Gt

Aidan Eichman (ane2123)
...
class SignLanguage:
    ...
    def create_model(self):
        ...
        # pulled from https://keras.io/examples/vision/mnist_convnet/
        ...
        # compiles the model with categorical_crossentropy
        model.compile('adam', loss=keras.losses.categorical_crossentropy, metrics=['accuracy'])

        # assigns the model to self.model
        self.model = model
    ...
    def prepare_data(self, images, labels):
        ...
        x_train, x_test, y_train, y_test = train_test_split(images, labels, random_state=100,test_size=0.23, shuffle=True)


        # reshape each example into a 2D image (28, 28, 1)
        # used https://www.javatpoint.com/numpy-reshape to allow numpy to fit an arbitrary number of images with -1
    ...
    def train(self, batch_size:int=128, epochs:int=50, verbose:int=1):
        ...
        # re-gather the x_train and y _train
        x_train, y_train = self.data["train"]


        # the history is the output of model.fit
        history = self.model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs, verbose=verbose, validation_split=0.12)
    ...
    def predict(self, data):
        ...
        # reshape data again to (28, 28, 1)
        data = data.reshape(-1, 28, 28, 1)

        # use predict and np argmax
        predicted_data = self.model.predict(data)
        outputted_labels = np.argmax(predicted_data, axis=1)

        return outputted_labels
